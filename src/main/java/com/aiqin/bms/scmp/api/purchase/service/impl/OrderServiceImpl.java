...
//        changeOrderStatusReqVO.setOrderStatus(ReturnOrderStatus.RETURN_COMPLETED.getStatusCode());
        ...
        //异步保存单据
        asynSaveDocuments.savePurchase(reqVO.getOrderCode());
        return Boolean.TRUE;
    }
...
//                System.out.println(JsonUtil.toJson(saleSourcInfoSource));
    ...
    private void batchProce(OrderInfoReqVO vo, WarehouseDTO warehouse) {
        ...
        if (org.apache.commons.collections.CollectionUtils.isNotEmpty(stockList) && stockList.size() > 0) {
            ...
            if (org.apache.commons.collections.CollectionUtils.isNotEmpty(stockDayBatches)) {
...
//            SimpleDateFormat sf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            ...
            if (productList != null && productList.size() > 0) {
                ...
                if (erpOrderItemMap.containsKey(key)) {
                    ...
                    BigDecimal fxAmount = item.getProductAmount() == null ? BigDecimal.ZERO : item.getProductAmount();
                    product.setPrice(fxAmount);
                    BigDecimal fxTotalAmount = fxAmount.multiply(BigDecimal.valueOf(item.getProductCount()));
                    product.setAmount(fxTotalAmount);
