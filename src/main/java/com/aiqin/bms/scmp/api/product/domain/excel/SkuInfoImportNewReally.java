...
@Data
@ApiModel("sku新增导入实体(真)")
public class SkuInfoImportNewReally extends BaseRowModel {

    public static final String HEAD = "SkuInfoImportNewReally(skuCode=SKU号, procurementSectionName=采购组, goodsGiftsDesc=类型, productSortName=所属部门, productCategoryName=品类编码, skuName=SKU名称, skuAbbreviation=SKU简称, mnemonicCode=SKU助记码, productName=所属SPU, spuAbbreviation=SPU简称, spuMnemonicCode=SPU助记码, styleNumber=款号, itemNumber=货号, productBrandName=品牌, productPropertyName=商品属性, colorName=颜色, modelNumber=型号, qualityDate=保质天数, applicableMonthAge=适用起始月龄, seasonalGoodsDesc=是否季节商品, warehouseTypeName=仓位类型, structuralGoodsDesc=结构性商品, useTime=使用时长, seasonBand=季节波段, uniqueCodeDesc=唯一码管理, level=等级, featureName=特征, currencyLevelName=通货等级, priceChannelName=覆盖渠道, tagName=商品标签, remark=商品备注, productDesc=商品介绍, manufacturerGuidePrice=厂商指导价, readyCol67=爱亲渠道价, readyCol68=萌贝树渠道价, readyCol69=小红马渠道价, readyCol70=爱亲分销价, readyCol71=萌贝树分销价, readyCol72=小红马分销价, readyCol74=售价, readyCol75=会员价, supplyUnitName=供应商, jointFranchiseRate=联营扣点, point=返点, factorySkuCode=供应商SKU编号, taxIncludedPrice=含税采购价, supplyCategoriesSupplyChannelsName=供货渠道类别, stockSpec=库存规格, stockUnitName=库存单位, stockBoxLength=库存长(MM), stockBoxWidth=库存宽(MM), stockBoxHeight=库存高(MM), stockBoxVolume=库存体积(MM3), stockBoxGrossWeight=库存毛重(KG), stockNetWeight=库存净重(KG), purchaseSpec=采购规格, purchaseUnitName=采购单位, purchaseBoxLength=采购长(MM), purchaseBoxWidth=采购宽(MM), purchaseBoxHeight=采购高(MM), purchaseBoxVolume=采购体积(MM3), purchaseBoxGrossWeight=采购毛重(KG), purchaseNetWeight=采购净重(KG), purchaseBaseProductContent=采购单位含量, purchaseBarCode=采购条形码, distributionZeroRemovalCoefficient=分销交易倍数, maxOrderNum=最大订购数量, saleBarCode=销售条形码, description=销售描述, settlementMethodName=结算方式, inputTaxRate=进项税率, outputTaxRate=销项税率, integralCoefficient=积分系数, logisticsFeeAwardRatio=物流费奖励比例, readyCol76=华北仓状态, readyCol77=华东仓状态, readyCol78=华南仓状态, readyCol79=西南仓状态, readyCol80=华中仓状态, manufacturerName=生产厂家, factoryProductNumber=生产厂家SKU编号, address=保修地址)";
