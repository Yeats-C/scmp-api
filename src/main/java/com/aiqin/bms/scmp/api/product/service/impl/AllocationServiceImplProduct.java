...
public class AllocationServiceImplProduct extends ProductBaseServiceImpl implements AllocationService  {
    ...
    public Long save(AllocationReqVo vo) {
        ...
        //设置状态
        allocation.setAllocationStatusCode(Byte.valueOf("0"));
        allocation.setAllocationStatusName(AllocationEnum.getAllocationEnumNameByCode(Byte.valueOf("0")));
        Long k  = ((AllocationService) AopContext.currentProxy()).insertSelective(allocation);
        try{
              //转化调拨单sku列表
            List<AllocationProduct>list = BeanCopyUtils.copyList(vo.getList(),AllocationProduct.class);
            list.stream().forEach(allocationProduct ->{allocationProduct.setAllocationCode(allocation.getAllocationCode());} );
             int kp =  ((AllocationService) AopContext.currentProxy()).saveList(list);
             if(kp>0){
    ...
    public String workFlowCallback(WorkFlowCallbackVO vo1) {
        ...
        List<AllocationProduct> list =  allocationProductMapper.selectByAllocationCode(allocation.getAllocationCode());
